...
class vtraflux_lmfit:
    ...
    def __init__(self, dream,
        ...
        self.dream       = dream
        #self.chain = self.dream.chains[0]
        ...
        #self.params   = self.ec_model.chain2paramdict(params, self.dream.chains[random.randint(0, self.dream.nc)], self.comp, self.w)
    ...
    def model_run(self): 
        ...
        #ec_model = model["EC"]
        #w        = model["wv"]
        #mini_res = minimize(self.objective, params, method=self.method, iter_cb = self.iter_fun )        
        #mini_res = minimize(self.objective, self.params, method=self.method,args = self.models, kws = self.comps)        
        ...
        #if self.mini_res.nfev >= 500:
        feval_count = 0
        #while self.mini_res.nfev >= 500 and feval_count <= 5:
        if self.npars > 20: 
            maxfev = 1000
        else: 
            maxfev = 500       
        ...
        while self.mini_res.nfev >= maxfev:
            ...
            #self.params   = self.ec_model.chain2paramdict(params,self.chain , self.comp, self.w)
            
            #if "T" in self.comp:
            #    self.params   = self.T_model.chain2paramdict(params, self.chain, self.comp, self.w)
            #if "EC" in self.comp: 
            #    self.params   = self.ec_model.chain2paramdict(params, self.chain, self.comp, self.w)
    ...
    def print_results(self):
        ...
        if "EC" in self.comp: 
            ...
            #postpars = np.append(np.copy(current), np.append(np.copy(rmse), np.append(np.copy(sse), np.append(np.copy(like), np.append(np.copy(oodlike), np.append(np.copy(vvdlike), np.copy(1) ))))))
