          ...
          for depthpair in depthpairs:
            ...
            for comps in compounds: 
              for h_factor in par_info['v_ints']:
                    ...
                    D,  dirName2s, indata, result_df, npars, seclocs , dirName_run = DREAM_ecmod_create(site, comps,  dat_info, par_info, run_info); R_hat = []
                    ...
                    print( 'psutil num_cpus= ',nprocs_avail) # very important to match number of chains to number of cores 
                    if run_info["supercomputer"]:
                        nprocs_avail = len(os.sched_getaffinity(0))  
                        print( 'supercomputer num_cpus= ',nprocs_avail) # very important to match number of chains to number of cores 
    
    
                    nprocs = len(weights)
                    print( 'num_cpus used = number of weights =  ', nprocs) # very important to match number of chains to number of cores 
                    ...
                    vtralm_models = [modified_script.vtraflux_lmfit.remote(dream = D[i], method = method,  models = models_dict_l[i],  comps = comps_dict, ID = i) for i in range(len(weights))]
