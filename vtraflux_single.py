        ...
        for comps in compounds: 
          for h_factor in par_info['v_ints']:
                ...
                for w in weights:
                    ...
                    D,  dirName2s, indata, result_df, npars, seclocs , dirName = DREAM_ecmod_create(site, comps,  dat_info, par_info, run_info); R_hat = []
                    ...
                    if run_info["vantage"]: 
                        for i in range(D.nc): D.chains[i].current=  np.array(result_df)[i][1:npars+1]
                    ...
                    print( 'nchains= ',D.nc) # very important to match number of chains to number of cores 
                    print( 'num_cpus used= ',nprocs) # very important to match number of chains to number of cores 
                    ...
                    if "EC" in comps: 
                        if "T" in comps:
                            ...
                            rbc = 0.0; 
                        ec_models = [vtraflux_model.remote(indata_EC,dat_info["chunk"],dt = 0.05, rbc = rbc,  seclocs= seclocs_EC, param_info = par_info,comp="EC", ID = i) for i in range(D.nc)]
                    ...
                    if "T" in comps:
                        if len(seclocs_T) > 2: 
                            ...
                            if "EC" in comps:
                                ...
                                T_models = [vtraflux_model.remote(indata_T,dat_info["chunk"],dt = 0.025, rbc = rbc,   seclocs= seclocs_T, param_info = par_info,comp="EC_T",ID = i) for i in range(D.nc)]
                            else:
                                T_models = [vtraflux_model.remote(indata_T,dat_info["chunk"],dt = 0.025, rbc = rbc,   seclocs= seclocs_T, param_info = par_info,comp="T",ID = i) for i in range(D.nc)]
                ...
                #%% #Burn in start
                    ...
                    while D.burn == True:   
                        ...
                        if D.ct != 0 and D.ct >= 5: 
                            D.Chain_removal(reset_removal, D.tc); # omega calculated using the last half of all likelihoods   
                        ...
                        if max(D.R) < 1.2 and D.ct > int(D.rac/100*D.nc): D.burn = False 
                        #print('chain End= ',time.strftime("%Y-%m-%d %H:%M:%S"))
                        if postrun: 
