...
def load_run_info(directory, folder, postest):
    ...
    try:
        ...
        return header, log10w ,vres, npars, current_header
...
def load_text_file(directory,folder, filename,postest):
    ...
    try:
        with open(file_path, 'r') as file:
            header, log10w ,vres, npars, current_header= load_run_info(directory, folder, postest = postest)
            #print(header)
            #print(len(header))
            ...
            #if postest == True and os.path.isfile(file_path2):
            ...
            if test== 0:
                ...
                diffs = vvs.diff(axis=1)
                realmeans = abs(vvs.mean(axis = 1))
                ...
                #osum =np.sum(np.array(diffs.fillna(0)**2), axis=1)
                #osum = np.sum(np.array((diffs.fillna(0))**2), axis=1)

                sumsum = vsum + osum
                #content.loc[content['pnorm']  == 0, "pnorm"] = sumsum
...
def estimate_w_old(result_df, smooth_l, dirName):
